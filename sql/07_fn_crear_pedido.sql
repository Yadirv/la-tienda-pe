-- =============================================================================
-- sql/07_fn_crear_pedido.sql
-- Función RPC atómica para crear pedidos B2C con control de inventario.
--
-- Garantías:
--   • SELECT ... FOR UPDATE → bloqueo de fila durante toda la transacción.
--   • Si stock < qty para cualquier item → ROLLBACK total (nada se inserta).
--   • UPDATE usa inventario_b2c - qty (resta en Postgres, no en el cliente).
--   • cc_nit = NULL para clientes B2C (no requiere registro en clientes_b2b).
--
